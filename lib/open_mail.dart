...
class OpenMail {
  ...
  /// are found will return a [OpenMailResult] that contains list of
  ...
  static Future<OpenMailResult> openMail({
    ...
    if (_isAndroid) {
      final result = await _channel.invokeMethod<bool>(
          ...
          false;
      return OpenMailResult(didOpen: result);
    } else if (_isIOS) {
      ...
      if (apps.length == 1) {
        ...
        return OpenMailResult(didOpen: false, options: apps);
      }
    } else {
  ...
  static Future<OpenMailResult> composeNewEmailInMailApp({
    String nativePickerTitle = '',
    ...
    if (_isAndroid) {
      ...
      if (installedApps.length == 1) {
        ...
        return OpenMailResult(didOpen: false, options: installedApps);
      }
    } else {
...
/// Use with [OpenMail.getMailApps] or [OpenMail.openMail] to get a
...
/// Result of calling [OpenMail.openMail]
///
/// [options] and [canOpen] are only populated and used on iOS
class OpenMailResult {
  final bool didOpen;
